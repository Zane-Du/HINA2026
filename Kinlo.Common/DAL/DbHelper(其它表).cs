...
public partial class DbHelper
{
    ...
    public async Task<List<MesResendModel>> QueryMesResendListAsync(int count, string logHeader) =>
        await _dbFactory.UsingDbAsync(DatabaseRole.LocalDb1, async db =>
        await OnQueryMesResendListAsync(db, count, logHeader)
      );
    ...
    private async Task<List<MesResendModel>> OnQueryMesResendListAsync(ISqlSugarClient db, int count, string logHeader)
    {
        try
        {
            var data = await db.Queryable<MesResendModel>().Where(x => x.ResendStatus != ResendStatusEnum.上传成功 && x.ResendCount <= 10).OrderBy(x => x.Id).Take(count).ToListAsync();
            if (data != null)
            {
    ...
    => await _dbFactory.UsingDbAsync(DatabaseRole.LocalDb1, async db
        => await OnRangeDeleteResendAsync(db, dayCount));
    ...
    {
        var tranResult = await _dbFactory.UsingTransactionAsync<bool>(DatabaseRole.LocalDb1, async db =>
        {
    ...
    /// <returns></returns>
    public async Task UpdatedMainTableMesStatusAsync(ISqlSugarClient db, List<IBatMainModel> bats, string logHeader, SqlSugarScope? tran = null)
    {
    ...
    /// <returns></returns>
    public async Task<List<T>?> GetStatisticsData<T>(DateTime startTime, DateTime endTime, List<string> selectFields) where T : class, new() => await _dbFactory.UsingDbAsync(DatabaseRole.LocalDb1, async db => await OnGetStatisticsData<T>(db, startTime, endTime, selectFields));

    private async Task<List<T>?> OnGetStatisticsData<T>(ISqlSugarClient db, DateTime startTime, DateTime endTime, List<string> selectFields) where T : class, new()
    {
        try
        {
    ...
    /// <param name="endTime"></param>
    /// <returns></returns>
    public async Task<List<PlcStatusModel>> GetTimeRangePlcStatusAsync(DateTime startTime, DateTime endTime, ShiftType? shift, DeviceStateEnum[]? deviceState) =>
      await _dbFactory.UsingDbAsync(
        DatabaseRole.LocalDb1,
        async db => await OnGetTimeRangePlcStatusAsync(db, startTime, endTime, shift, deviceState)
      );

    public async Task<List<PlcStatusModel>> OnGetTimeRangePlcStatusAsync(ISqlSugarClient db, DateTime startTime, DateTime endTime, ShiftType? shift, DeviceStateEnum[]? deviceState)
    {
        try
        {
            ...
            for (int i = 0; i < yearCount; i++)
            {
                ...
                var yearQueryable = db.Queryable<PlcStatusModel>().AS(tableName).Where(x => x.StartTime >= startTime && x.StartTime <= endTime).WhereIF(shift.HasValue, x => x.Shift == shift!.Value).WhereIF(deviceState != null && deviceState.Length > 0, x => deviceState.Contains(x.Status));
        ...
        {
            $"[获取时间范围内PLC状态数据]异常,开始时间：{startTime:yyyy-MM-dd HH:mm:ss},结束时间：{endTime:yyyy-MM-dd HH:mm:ss}，详情：{ex}".LogRun(Log4NetLevelEnum.错误);
        }
        return new List<PlcStatusModel>();
    }
    #endregion
    ...
    public async Task<List<PlcAlarmModel>> OnGetTimeRangePlcAlarmsAsync(ISqlSugarClient db, DateTime startTime, DateTime endTime, PlcAalrmLevelEnum[] types)
    {
        try
        {
            long startId = _snowflakeHelper.GetMinIdFromDateTime(startTime);
            ...
            for (int i = 0; i < monthCount; i++)
            {
                ...
                var monthQueryable = db.Queryable<PlcAlarmModel>().AS(tableName).Where(x => x.Id >= startId && x.Id <= endId && types.Contains(x.PlcAalrmLevel));
        ...
        {
            $"[获取时间范围内PLC报警数据]异常,开始时间：{startTime:yyyy-MM-dd HH:mm:ss},结束时间：{endTime:yyyy-MM-dd HH:mm:ss}，详情：{ex}".LogRun(Log4NetLevelEnum.错误);
        }
