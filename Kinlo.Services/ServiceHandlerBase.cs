...
public abstract class ServiceHandlerBase : IServiceHandler
{
    ...
    protected ServiceHandlerBase(  IContainer container, IDevice plc, PLCInteractAddressModel plcInteractAddress,  CancellationTokenSource taskToken )
    {
        ...
        #region 判断当前工序是否进出站
        var group = _plcSignalConfig.PLCInteractAddresses.Where(x => x.ProcessesType != ProcessTypeEnum.设备状态).GroupBy(x => x.ProcessesType)  .ToList();
        ...
        {
            if ( _plcSignalConfig.PLCInteractAddresses.Where(x => x.ProcessesType != ProcessTypeEnum.设备状态)  .MinBy(x => x.ProductionIndex)  ?.ProcessesType == Context.ProcessesType  )
            {
    ...
    protected (bool status, bool[] content) GetLaneBatteryStatus(bool isReadPlcSuccess, List<ReceivePlcDataModel> plcDatas, string logHeader)
    {
        ...
        else
        {
            if (!isReadPlcSuccess)
            {
                $"已启用PLC指定通道是否有电池，但PLC未给数据，不给PLC发结果及完成信号，请停机检查！;".LogProcess(logHeader, Log4NetLevelEnum.错误, true);
