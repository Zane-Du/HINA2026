...
public class SendTrayIDHandler : ServiceHandlerBase
{
    ...
    public SendTrayIDHandler( IContainer container, IDevice plc, PLCInteractAddressModel plcInteractAddress, CancellationTokenSource taskToken) : base(container, plc, plcInteractAddress, taskToken) { }
    ...
    protected override Task HandleCore(short plcValue)
    {
        ...
        new SignalAddressModel($"{Context.DataAddress.Lable}.ToPLCData[0]").WritePlcResult(   string.IsNullOrEmpty(_strCode) ? ResultTypeEnum.NG : ResultTypeEnum.OK,   ResultTypeEnum._,  _plc,   _parameterConfig,  _taskLogHeader );
