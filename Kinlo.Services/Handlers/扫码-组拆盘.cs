...
{
    #region 构造函数方法
    public ScanCodeLoadCrateHandler(IContainer container, IDevice plc, PLCInteractAddressModel plcInteractAddress, CancellationTokenSource taskToken) : base(container, plc, plcInteractAddress, taskToken) { }
    #endregion
    ...
    protected override Task HandleCore(short plcValue)
    {
        ...
        #region 执行扫码方法，拿到扫码结果
        var isHaveBatterys = Enumerable.Repeat(true, Context.DataLength).ToArray();
        var scanResults = ScanBarcodeHelper.ScanCode(device, _parameterConfig, Context, isHaveBatterys, logHeader, _parameterConfig.AdvancedConfig.LogisticsTrayCodeValidationRule);
