...
{
    #region 构造函数方法
    public RJ6902CAGXHandler(IContainer container, IDevice plc, PLCInteractAddressModel plcInteractAddress, CancellationTokenSource taskToken) : base(container, plc, plcInteractAddress, taskToken) { }
    ...
    protected override async Task HandleCore(short plcValue)
    {
        ...
        await Parallel.ForEachAsync(plcDatas, new ParallelOptions { MaxDegreeOfParallelism = Context.DataLength }, async (plcData, _) =>
        {
            ...
            OperationResult<RJ6902CAGXResultModel> deviceResult = OperationResult<RJ6902CAGXResultModel>.Failure("默认");
            try
            {
            ...
            #region 如果软路测试结果不合格，立刻执行上传MES方法
