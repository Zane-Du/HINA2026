   ...
   protected override async Task HandleCore(short plcValue)
   {
      ...
      await Parallel.ForAsync(
         ...
         async (i, _) =>
         {
            ...
            {
               $"扫码最大个数[{scanCodeResults.Length}] 小于设定个数[{i + 1}]!".LogProcess(
                  logHeader,
                  Log4NetLevelEnum.错误,
                  true
               );
            ...
            for (int n = 1; n < 4; n++)
            {
               ...
               {
                  $"第[{n}]次 地址[{JsonSerializer.Serialize(tag)}]写入条码[{batMain.Barcode}] 失败".LogProcess(
                     logHeader
                  );
               }
            }
            ...
            resultPath.WritePlcResult(
               batReworkScan.ReworkScanResult,
               ResultTypeEnum._,
               _plc,
               _parameterConfig,
               logHeader
