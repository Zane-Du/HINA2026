...
public class BackHeliumAndBallHandler : ServiceHandlerBase
{
    ...
    public BackHeliumAndBallHandler(IContainer container, IDevice plc, PLCInteractAddressModel plcInteractAddress, CancellationTokenSource taskToken) : base(container, plc, plcInteractAddress, taskToken)
    {
        /// <summary>
        /// 读取PLC数据的长度，由配置中读取
        /// </summary>
    ...
    protected override async Task HandleCore(short plcValue)
    {
        ...
        {
            string _msg = $"==>>{Context.ProcessesType}<<==\r\n收到启动信号，没收到电池记忆，请联系电气工程师！！！\r\n标签：{JsonSerializer.Serialize(Context.DataAddress)}";
            UIThreadHelper.InvokeOnUiThreadAsync(new Action(() => Dialog.Show(new AlarmDialog(new AlarmDialogDto(_msg)), GenericHelper.FullScreenAlarmToken)));
        ...
        //遍历所有ID
        await Parallel.ForAsync(0, helium.Actuals.Length, new ParallelOptions { MaxDegreeOfParallelism = Context.DataLength }, async (dtIndex, _) =>
        {
            ...
            if (plcBall.Actuals.Length > dtIndex)
            {
                ...
                var ballReulst = backHelium.BallHorizontalOffsetReslut == ResultTypeEnum.OK && backHelium.BallVerticalOffsetReslut == ResultTypeEnum.OK ? ResultTypeEnum.OK : ResultTypeEnum.NG;
            }
            else
            {
            ...
            #region 如果回氦打钢珠结果不合格，立刻执行MES上传方法
            if (backHelium.BackHeliumResult != ResultTypeEnum.OK || backHelium.BallHorizontalOffsetReslut != ResultTypeEnum.OK || backHelium.BallVerticalOffsetReslut != ResultTypeEnum.OK)
            {
